...
class PypikaRepository(AbstractRepository):
    ...
    def get(self, entity_model: Type[Entity], entity_id: Union[str, int]) -> Entity:
        ...
        """
        table = self.table_model(entity_model)
        query = Query.from_(table).select("*").where(table.ID == entity_id)

        try:
            return self.build_entities(entity_model, query)[0]
    ...
    def all(self, entity_model: Type[Entity]) -> List[Entity]:
        ...
        entities = self.build_entities(entity_model, query)
        if len(entities) == 0:
            raise EntityNotFoundError(
    ...
    def build_entities(self, entity_model: Type[Entity], query: Query) -> List[Entity]:
